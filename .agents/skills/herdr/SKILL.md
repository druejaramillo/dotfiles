...
For a research workflow that explicitly requests independent evidence gathering or review, use the general procedure above rather than assuming every helper is a Pi process. If the user has not selected another kind and this is the user's Pi research workflow, `--kind pi` uses the normal Pi setup without fixed researcher/reviewer personas. A helper is a fresh process: pass a bounded question and the necessary source or draft paths, not the parent conversation or credentials. For a review, ask for read-only, source-linked findings and keep the parent agent responsible for checking and incorporating them. Do not have two agents edit the same artifact at once.

If the guard fails or Herdr work is blocked, the calling research workflow may continue with its own transparent, non-independent check; the Herdr skill itself must not attempt to control a session from outside Herdr. A second agent can incur separate model usage.
