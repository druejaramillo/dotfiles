...
- User skill: `~/.agents/skills/revealjs/scripts/create-presentation.js`
...
**IMPORTANT: edit existing slides incrementally** — one or a few slides at a time. Do not rewrite the entire HTML file when a targeted edit will work. The scaffold generates unique placeholder text per slide (e.g., `Slide 2 Title Here`), so each section can be targeted with Edit. This is more token-efficient and less error-prone than generating the full file at once.
